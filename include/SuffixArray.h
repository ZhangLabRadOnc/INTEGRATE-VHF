...
template <typename T> class SuffixArray {
  public:
    ...
    explicit SuffixArray(std::string const &sequence) : _sa(sequence.size(), 0), _decimation(1) { divsufsort_wrapper(sequence, _sa.data()); }
    ...
    value_type operator[](size_t idx) const { return _sa[idx]; }

    size_t size() const { return _sa.size(); }

    bool empty() const { return _sa.empty(); }

    size_t capacity() const { return _sa.capacity(); }

    bool operator==(SuffixArray const &rhs) const { return _sa == rhs._sa; }
    ...
    size_t decimation() const { return _decimation; }

    value_type const *data() const { return _sa.data(); }
