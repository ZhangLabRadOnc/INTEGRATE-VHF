...
int BreakPoint::getBreakPoints(vector<break_point_record_t> &bkvec, const char *filename, const char *filename1, const char *filename2, const char *refname, Reference &ref, const char *sample_name) {
    ...
    for (int i = 0; i < vcfvec.size(); i++) {
        vcf_t vt = vcfvec[i];
        outFile2 << vt.chr << "\t" << vt.pos << "\t" << vt.id << "\t" << vt.ref << "\t" << vt.alt << "\t" << vt.qual << "\t" << vt.filter << "\t" << vt.info << "\t" << vt.format
                 << "\t" << vt.content << endl;
    }
    return 0;
}
...
int BreakPoint::printOneBK(break_point_record_t &bkt, Reference &ref, ofstream &outFile) {
    outFile << bkt.nm5p << "\t";
    outFile << bkt.nm3p << "\t";
    outFile << ref.getSeqMappedName(bkt.tid1) << "\t";
    outFile << bkt.rnabk1 << "\t";
    if (bkt.isExon == 1) {
        outFile << bkt.exonbk1 << "\t";
    } else
        outFile << "NA"
                << "\t";
    outFile << ref.getSeqMappedName(bkt.tid2) << "\t";
    outFile << bkt.rnabk2 << "\t";
    if (bkt.isExon == 1) {
        outFile << bkt.exonbk2 << "\t";
    } else {
...
int BreakPoint::AddOneVCF(break_point_record_t &bkt, Reference &ref) {
    ...
    if (imprecise == 0 && bkt.tid1 == bkt.tid2 && bkt.gStrand1 == 0 && bkt.gStrand2 == 0 && bkt.exonbk1 < bkt.exonbk2 && bkt.dnabk1 < bkt.dnabk2) {
        vcf_t vt_del;
        vt_del.chr = ref.getSeqMappedName(bkt.tid1);
    ...
    if (imprecise == 0 && bkt.tid1 == bkt.tid2 && bkt.gStrand1 == 1 && bkt.gStrand2 == 1 && bkt.exonbk1 > bkt.exonbk2 && bkt.dnabk1 > bkt.dnabk2) {
        vcf_t vt_del;
        vt_del.chr = ref.getSeqMappedName(bkt.tid2);
    ...
    // bnd
    vcf_t vt;
    vt.chr = ref.getSeqMappedName(bkt.tid1);
    uint32_t pos1 = 0;
    if (imprecise) {
        if (bkt.isExon == 1)
            pos1 = bkt.exonbk1;
    ...
    vt2.chr = ref.getSeqMappedName(bkt.tid2);
    uint32_t pos2 = 0;
    if (imprecise) {
        if (bkt.isExon == 1)
            pos2 = bkt.exonbk2;
