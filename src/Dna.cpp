...
bool isEqLocal(encompass_dna_t &i, encompass_dna_t &j, Gene &g) {
    if (((j.pos2 == i.pos2 && j.pos1 == i.pos1 && j.tid2 == i.tid2 && j.tid1 == i.tid1) || (j.pos1 == i.pos2 && j.pos2 == i.pos1 && j.tid1 == i.tid2 && j.tid2 == i.tid1)) &&
...
int Dna::getCandFromDNA(const char *tmpfile, Gene &g) {
    ...
    if (pFile == nullptr)
        ...
        while (!feof(pFile)) {
            if (fgets(buffer, 1024, pFile) == nullptr)
                break;
            sscanf(buffer, "%s\t%s\n", name1, name2);
            ids.clear();
            ids2.clear();
...
int getRefTid1(int enId) { return entmp2[enId].tid1; }
...
int getRefTid2(int enId) { return entmp2[enId].tid2; }
...
int Dna::printSpDna(Gene &g, Reference &ref) {
    ...
    for (int i = 0; i < spdna.size(); i++) {
        ...
        cout << st.strand1 << "\t";
        cout << ref.getSeqName(st.tid1);
        cout << st.pos1 << "\t";
        cout << st.len1 << "\t";

        cout << st.strand2 << "\t";
        cout << ref.getSeqName(st.tid2);
        cout << st.pos2 << "\t";
        cout << st.len2 << "\t";
