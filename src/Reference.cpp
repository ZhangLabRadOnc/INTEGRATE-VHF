...
inline int getMask(char a) { return mask1[a]; }
...
char getCharFromBinary(int a) { return charMap[a]; }
    ...
    */
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (last == 8 && i + 7 < len) {
            ...
            reference[numInRef] = (getMask(seq[i]) << 28) | (getMask(seq[i + 1]) << 24) | (getMask(seq[i + 2]) << 20) | (getMask(seq[i + 3]) << 16) | (getMask(seq[i + 4]) << 12) |
                                  (getMask(seq[i + 5]) << 8) | (getMask(seq[i + 6]) << 4) | (getMask(seq[i + 7]));
...
string Reference::getCharName(int i) { return this->nameList[i]; }

uint32_t Reference::getPosLeft(int i) { return this->posListLeft[i]; }

uint32_t Reference::getPosRight(int i) { return this->posListRight[i]; }

int Reference::getListSize() { return this->nameList.size(); }
...
uint32_t Reference::getRefLength() { return refLength; }
