...
int printOneEncompassDna(encompass_dna_t &et, Reference &ref, ofstream &outFile) {
    ...
    if (et.strand1 == 0)
        ...
        outFile << "-";
    outFile << "\t";
    outFile << ref.getSeqOriginalName(et.tid1);
    outFile << "\t";
    outFile << et.pos1;
    outFile << "\t";
    outFile << et.len1;
    ...
    if (et.strand2 == 0)
        ...
        outFile << "-";
    outFile << "\t";
    outFile << ref.getSeqOriginalName(et.tid2);
    outFile << "\t";
    outFile << et.pos2;
    outFile << "\t";
    outFile << et.len2;
...
bool my_sort_int(int i, int j) { return i < j; }
bool my_sort_string(string i, string j) { return i.compare(j); }
...
int Result::getSize() { return results.size(); }
...
int Result::printExons(const char *filename, Gene &g, Reference &ref, int isRunningNormal, const char *bkfile, const char *bkfileBEDPE, const char *bkfileVCF, const char *refname, const char *sample_name) {
    ...
    outFile << "#Id\t5p\t3P\t5P_Transcipt\t5P_Exon\t5P_Strand\t5P_Exon_Chr\t5P_Exon_Start\t5P_Exon_End\t5P_Exon_Seq\t5P_Exon_150\t3P_Transcript\t3P_Exon\t3P_Exon_Strand\t3P_Exon_"
               "Chr\t3P_Exon_Start\t3P_Exon_END\t3P_Exon_Seq\t3P_Exon_150"
            << endl;
...
<< " realPrint=" << prt->realPrint << endl;
        ...
        if (is5p == 1 && is5p_2 == 0) {
            ...
            if (strand == 0)
                ...
                outFile << "-\t";
            outFile << ref.getSeqOriginalName(tid) << "\t";
            outFile << pos1 << "\t" << pos2 << "\t";
            ...
            if (strand_2 == 0)
                ...
                outFile << "-\t";
            outFile << ref.getSeqOriginalName(tid_2) << "\t";
...
int Result::getAllJunctionsStep1(Gene &g, Reference &ref) { return 0; }
...
int Result::getAllJunctionsStep6(const char *filename, Gene &g, Reference &ref) {
    ...
    for (int i = 0; i < fjtvec2.size(); i++) {
        ...
        if (last_pos5 != p5_pos || last_pos3 != p3_pos) {
            ...
            // cout<<"here1.2"<<endl;
            if (fjtvec2[i].isCano == 1)
                outFile << ref.getSeqOriginalName(fjtvec2[i].p3.tid) << "\t";
            else
                outFile << ref.getSeqOriginalName(g.getTid(fjtvec2[i].geneId3p)) << "\t";
