...
void Rna::processEnReads(TidHandler &th, Gene &g, unordered_map<string, pair<bam1_t *, bam1_t*>> &readGroups) {
    for (auto it = readGroups.begin(); it != readGroups.end(); it++) {
        ...
        if(b1->core.l_qseq == 0 || b2->core.l_qseq == 0 || 
            (b1->core.flag & BAM_FUNMAP) || (b2->core.flag & BAM_FUNMAP))
        {
            bam_destroy1(b1);
            bam_destroy1(b2);
            continue;
...
//cout<<"case 2 "<<endl;
                    ...
                    for(int k=0;k<npm1;k++)
                    {
                        ...
                        if(parnaM[pidm1].strand==parna[pid1].strand)
                        {
                            if(parna[pid1].map_case==0)
                            {
                                if(parna[pid1].a<parnaM[pidm1].b+5 && parna[pid1].a>parnaM[pidm1].b-5 && seqLen-parna[pid2].a<parnaM[pidm1].a+5 &&
seqLen-parna[pid2].a>parnaM[pidm1].a-5 && g.getEndPos(parnaM[pidm1].transIds[0],parnaM[pidm1].exonIds[0]) < g.getStartPos(parna[pid1].transIds[0],parna[pid1].exonIds[0]))
                                {
                                    isadd=1;
                                }
                            }
                            else
                            {
                                if(parna[pid1].b<parnaM[pidm1].a+5 && parna[pid1].b>parnaM[pidm1].a-5 && seqLen-parna[pid2].b-1<parnaM[pidm1].b+5 &&
                                        seqLen-parna[pid2].b-1>parnaM[pidm1].b-5 && g.getStartPos(parnaM[pidm1].transIds[0],parnaM[pidm1].exonIds[0]) >
g.getEndPos(parna[pid1].transIds[0],parna[pid1].exonIds[0]))
                                {
                                    isadd=1;
                                }
                            }
                    ...
                    for(int k=0;k<npm2;k++)
                    {
                        ...
                        if(parnaM[pidm2].strand==parna[pid2].strand)
                        {
                            if(parna[pid1].map_case==0)
                            {
                                if(parna[pid2].a<parnaM[pidm2].b+5 && parna[pid2].a>parnaM[pidm2].b-5 && seqLen-parna[pid1].a<parnaM[pidm2].a+5 &&
seqLen-parna[pid1].a>parnaM[pidm2].a-5 && g.getEndPos(parnaM[pidm2].transIds[0],parnaM[pidm2].exonIds[0]) < g.getStartPos(parna[pid2].transIds[0],parna[pid2].exonIds[0]))
                                {
                                    isadd=1;
                                }
                            }
                            else
                            {
                                if(parna[pid2].b<parnaM[pidm2].a+5 && parna[pid2].b>parnaM[pidm2].a-5 && seqLen-parna[pid1].b-1<parnaM[pidm2].b+5 &&
                                        seqLen-parna[pid1].b-1>parnaM[pidm2].b-5 && g.getStartPos(parnaM[pidm2].transIds[0],parnaM[pidm2].exonIds[0]) >
g.getEndPos(parna[pid2].transIds[0],parna[pid2].exonIds[0]))
                                {
                                    isadd=1;
                                }
                            }
...
int Rna::printSplits() {
    ...
    for (int i = 0; i < sprna.size(); i++) {
        ...
        cout << sprna[i].strand2 << " " << sprna[i].tid2 << " " << sprna[i].pos2 << " " << sprna[i].len2 << " " << sprna[i].hits << " " << sprna[i].geneId1 << " "
             << sprna[i].geneId2 << endl;
    }
    return 0;
}
/*
    ...
    for (int i = 0; i < tmp2.size() - 1; i++) {
        ...
        for (int j = i + 1; j < tmp2.size(); j++) {
            if (tmp2[i].name.compare(tmp2[j].name) != 0 &&
                ((tmp2[i].pos1 == tmp2[j].pos1 && tmp2[i].pos2 == tmp2[j].pos2 && tmp2[i].len1 == tmp2[j].len1 && tmp2[i].len2 == tmp2[j].len2) ||
                 (tmp2[i].pos1 == tmp2[j].pos2 && tmp2[i].pos2 == tmp2[j].pos1 && tmp2[i].len1 == tmp2[j].len2 && tmp2[i].len2 == tmp2[j].len1))) {
                isM = 1;
            }
        }
        if (isM)
...
int Rna::countSp(vector<int> &spIds) {
    ...
    for (int i = 1; i < tmpen.size(); i++) {
        // cout<<tmpen[i].tid1<<" "<<tmpen[i-1].tid1<<" "<<tmpen[i].tid2<<" "<<tmpen[i-1].tid2<<" "<<tmpen[i].pos1<<" "<<tmpen[i-1].pos1<<" "<<tmpen[i].pos2<<"
        // "<<tmpen[i-1].pos2<<endl;
        if (tmpen[i].tid1 != tmpen[i - 1].tid1 || tmpen[i].tid2 != tmpen[i - 1].tid2 || tmpen[i].pos1 > tmpen[i - 1].pos1 + 10 || tmpen[i].pos1 + 10 < tmpen[i - 1].pos1 + 10 ||
            tmpen[i].pos2 > tmpen[i - 1].pos2 + 10 || tmpen[i].pos2 + 10 < tmpen[i - 1].pos2)
            count++;
    }
...
struct ComputeWeights1 { // works with or without star
    ComputeWeights1(ALGraph<int, FusionEdge> &graph, vector<encompass_rna_t> const &enrna, vector<split_rna_t> const &sprna) : graph(graph), enrna(enrna), sprna(sprna) {}
...
struct TraverseNum {
    TraverseNum(HitsCounter &hc, Rna &rna) : hc(hc), rna(rna) {}
...
struct WeightLessThan {
    WeightLessThan(double minWeight) : minWeight(minWeight) {}

    bool operator()(FusionGraph::edge_iterator edge) { return edge->second.weight < minWeight; }
...
int Rna::checkSame(bam1_t *b, vector<map_emt_t2> &mets, vector<map_emt_t2> &metsM, vector<map_emt_t2> &mets2, vector<map_emt_t2> &metsM2) {
...
int Rna::checkSame(int len, vector<map_emt_t2> &mets, vector<map_emt_t2> &metsM, vector<map_emt_t2> &mets2, vector<map_emt_t2> &metsM2) {
...
int Rna::matchParials(Gene &g, int gid1, int gid2, bam1_t *b, vector<map_emt_t2> &mets, vector<map_emt_t2> &metsM, vector<map_emt_t2> &mets2, vector<map_emt_t2> &metsM2, int count,
                      myFind2 &mf2) {
    ...
    for (int i = 0; i < mets.size(); i++) {
        int strand1 = mets[i].strand;
        int len1 = mets[i].b - mets[i].a + 1;
        cout << "Length of1: " << mets[i].tid << " " << len1 << " " << mets[i].b << " " << mets[i].a << " " << endl;
        ...
        for (int j = 0; j < mets2.size(); j++) {
            ...
            int len2 = mets2[j].b - mets2[j].a + 1;
            cout << "Length of2: " << mets2[j].tid << " " << len2 << " " << mets2[j].b << " " << mets2[j].a << " " << endl;
                ...
                if (homoTest2(g, st, mf2) != 1) {
                    ...
                    if (!isl && !isa1) {
                        ...
                        else
                        {
                            ...
                            if (seq1Str == seq2Str)
                            {
                                int adjustment = (diff1 <= 0  || abs(diff1) >= st.seq.size()) ? diff0 : diff1;
                                if (!st.reversed)
                                {
                                    if (st.strand1 == 0)
                                    {
                            ...
                            else
                            {
                                ...
                                // lambda to safely extract substring into seqRead
                                auto safe_substr = [&](int start, int length) -> bool {
                                if (start < 0 || start >= n_seqReadLen) {
                                    cerr << "Warning: Invalid start position for substr: " << start << ", seqLen: " << n_seqReadLen << ", nm: " << nm << endl;
                                    return false;
                                }
                                if (start + length > n_seqReadLen) {
                                    cerr << "Warning: start + length exceeds sequence size. start: " << start << ", length: " << length << ", seqLen: " << n_seqReadLen << ", nm: " << nm << endl;
                                    return false;
                                }
                                seqRead = seqRead.substr(start, length);
                                return true;
                                };
                                ...
                                //     }
                                    if (diff2 < 0)
                                    {
                                        int start = st.len2 + diff0;
                                        int length = abs(diff0);

                                        if (start < 0) continue;

                                        if(!safe_substr(start,length))                               
                                        { sprna.push_back(st);
                                            rnafg->addSpanning(gid1, gid2, sprna.size() - 1);
                                            added = true;
                                            return added;
...
bool isHardClip(const bam1_t *b) {
    ...
    for (int i = 0; i < nc; i++) {
        ...
        if (op == BAM_CHARD_CLIP) {
            int len = cigar[i] >> 4;//changed nc-1 to i
            if (len > largeClip) {
                // posM = i;
                largeClip = len;
            }
...
static int myGetOtherSeq(const bam1_t *b) {
    ...
    if (isHardClip(b) && tmpspv[to_change_tmpspv].lenC == 0 && tmpspv[to_change_tmpspv].name.compare(name_cand) == 0 && rgpartial.tid == mtid && rgpartial.lpos <= mpos &&
        rgpartial.rpos >= mpos) {
