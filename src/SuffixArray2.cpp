...
static void radixPass(int *a, int *b, int *r, uint32_t n, uint32_t K) { // count occurrences
...
void suffixArray(int *s, int *SA, uint32_t n, uint32_t K) {
    ...
    for (uint32_t i = 0, j = 0; i < n + (n0 - n1); i++)
        if (i % 3 != 0)
            s12[j++] = i;
...
int SuffixArray2::getSA(int i) { return array[i]; }
